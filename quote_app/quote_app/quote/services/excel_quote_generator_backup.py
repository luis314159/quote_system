...
# Importar el conversor de PDF
try:
    # Intentar primero el método basado en WeasyPrint
    from ..utils.pdf_converter import convert_excel_to_pdf
    logger.info("Using WeasyPrint PDF converter")
except ImportError:
    try:
        # Si falla, usar nuestro método alternativo
        from ..utils.pdf_converter_alternative import PDFConverter
        logger.info("Using alternative PDF converter")
        
        def convert_excel_to_pdf(excel_data, filename_prefix="quote"):
            return PDFConverter.convert_excel_to_pdf(excel_data, filename_prefix)
    except ImportError:
        logger.warning("No PDF converter available. PDF generation will be disabled.")
...
class ExcelQuoteGenerator:
    def __init__(self, user, project_data, is_internal=False, output_format='excel'):
        self.user = user
        self.company = user.company
        self.project_data = project_data
        self.is_internal = is_internal
        self.output_format = output_format.lower()  # 'excel' or 'pdf'
        
        # Validar el formato de salida
        if self.output_format not in ['excel', 'pdf']:
            logger.warning(f"Invalid output format: {self.output_format}. Using 'excel' as default.")
            self.output_format = 'excel'
            
        # Verificar si la conversión a PDF está disponible
        if self.output_format == 'pdf' and convert_excel_to_pdf is None:
            logger.warning("PDF conversion not available. Falling back to Excel format.")
            self.output_format = 'excel'
